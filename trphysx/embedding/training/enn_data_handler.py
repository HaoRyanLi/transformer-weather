    ...
    def createTrainingLoader(self, 
        ...
        with h5py.File(file_path, "r") as f:
            ...
            for key in f.keys():
                ...
                # Stride over time-series
                print(f"key: {key}, data_series.size: {data_series.detach().size()}, block_size: {block_size}, stride: {stride}.")
                for i in range(0, data_series.size(0) - block_size + 1, stride):  # Truncate in block of block_size
...
class ERA5DataHandler(EmbeddingDataHandler):
    """
    Built in embedding data handler for the ERA5 data
    """
    class ERA5Dataset(Dataset):
        """
        Dataset for ERA5 data. Dynamically loads data from file each
        mini-batch since loading an entire data-set would be way too large. This data-set
        support the loading of sub-chunked time-series.
        ...
        def __init__(self, 
            ...
            self.block_size = block_size
            self.extra_var_list = []
            if args.extra_var_list:
                extra_var = np.load(args.extra_var_paths)
                if 'lat' in args.extra_var_list:
                    lat = torch.linspace(-np.pi/2, np.pi/2, args.img_height+1)
                    lat = (lat[1:] + lat[:-1])*0.5
                    cos_lat = torch.reshape(torch.cos(lat), (-1,1))
                    area_weight = (cos_lat*args.img_height/torch.sum(cos_lat))
                    area_weight = torch.tile(area_weight, (1,args.img_width))
                    self.extra_var_list.append(area_weight)
                if 'geopoential' in args.extra_var_list:
                    self.extra_var_list.append(torch.FloatTensor(extra_var['geopoential']))
                if 'land' in args.extra_var_list:
                    self.extra_var_list.append(torch.FloatTensor(extra_var['land']))
            if  self.extra_var_list:
                self.extra_vars = torch.stack(self.extra_var_list, axis=0)
            else:
    ...
    @dataclass
    class ERA5DataCollator:
        """Data collator for the ERA5 data embedding
        """
        # Default collator
        def __call__(self, examples:List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
    ...
    def createTrainingLoader(self,
        ...
        shuffle: bool = True,
        mpi_rank: int = -1,
        mpi_size: int = 1) -> DataLoader:
        """Creating training data loader for the ERA5 data.
        For a single training simulation, the total time-series is sub-chunked into
        smaller blocks for training. This particular dataloader support splitting the
        dataset between GPU processes for parallel training if needed. 
        ...
        if mpi_size > 1:
            ...
            dataset = self.ERA5Dataset(file_path, [key[i] for i in indexes], [idx[i] for i in indexes], block_size)
        else:
            dataset = self.ERA5Dataset(file_path, key, idx, block_size)

        data_collator = self.ERA5DataCollator()
        training_loader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, drop_last=True, collate_fn=data_collator, num_workers=1)
